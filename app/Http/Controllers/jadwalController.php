...
class jadwalController extends Controller
{
    ...
    public function index()
    {
		...
		$hari = Hari::all();
		$ruangan = Ruangan::all();
        return view('jadwal.index', compact('guru', 'mata_pelajaran', 'kelas', 'hari', 'ruangan'));
    }
    ...
    public function all()
    {
        ...
        $emps = Jadwal::with('guru', 'mata_pelajaran', 'kelas')->get();
        $output = '';
        $p = 1 ;
        if ($emps->count() > 0) {
	...
	public function jadwal_semua_siswa($id)
	{
		...
		$rincian_siswa = Rincian_Siswa::with('siswa', 'jadwal')->where('jadwal_id', $jadwal->id)->get();
		$count = Rincian_Siswa::with('siswa', 'jadwal')->where('jadwal_id', $jadwal->id)->count();
		return view('jadwal.semua_siswa', compact('rincian_siswa', 'jadwal', 'count'));
	}
    ...
    {
        $id = $request->id;
        $emp = Jadwal::with('guru', 'mata_pelajaran', 'kelas')->where('id', $id)->first();
